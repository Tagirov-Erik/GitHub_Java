...
public class Group implements Voenkom{
	...
	public Student[] getSoldiers() {
		Student[] soldiers = new Student[10];
		for (int i = 0; i < this.arrayStudent.length; i++) {
			if ((this.arrayStudent[i] != null) && (this.arrayStudent[i].getSex() == "male") && (this.arrayStudent[i].getAge() > 18)) {
				soldiers[i] = this.arrayStudent[i];
			}
		}
		Arrays.sort(soldiers, new SortByLastName());
		return soldiers;
	}
}
